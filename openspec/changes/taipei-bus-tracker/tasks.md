...
- [ ] 4.1 設定 React Router，建立頁面結構：首頁（收藏）、搜尋頁、路線詳情頁
- [ ] 4.2 實作路線搜尋元件：搜尋框 + 即時匹配結果列表
- [ ] 4.2.1 [TDD] 搜尋元件測試：輸入關鍵字顯示匹配結果、空輸入清空列表、點擊結果導航、無結果提示
- [ ] 4.3 實作方向選擇元件：顯示去程/回程兩個方向供選擇
- [ ] 4.4 實作站點列表元件：顯示所有站的到站資訊（站名、ETA 狀態、車牌）
- [ ] 4.4.1 [TDD] ETA 狀態渲染測試：eta=300→"約5分"、eta=60→"進站中"、eta=-1→"未發車"、eta=-2→"末班車已駛離"、eta=-3→"交管不停靠"、eta=-4→"未營運"、有車牌時顯示車牌
- [ ] 4.5 實作 useEta hook：每 15 秒自動輪詢，頁面不可見時停止
- [ ] 4.5.1 [TDD] useEta hook 測試：掛載後立即請求、15 秒自動輪詢、頁面不可見時停止、重新可見時恢復、API 錯誤處理、unmount 清理 timer

## 5. React PWA 前端 - 收藏與通知

- [ ] 5.1 實作收藏功能：新增/移除收藏到 localStorage，收藏「路線+方向+站」組合
- [ ] 5.1.1 收藏功能測試：新增收藏寫入 localStorage、重新載入還原收藏、移除收藏同步更新 localStorage 與畫面
- [ ] 5.2 實作首頁收藏面板：顯示所有收藏站的即時到站狀態（批次查詢）
- [ ] 5.3 實作到站通知：設定 X 分鐘前提醒，ETA 到達時觸發 Browser Notification
- [ ] 5.3.1 [TDD] 通知觸發測試：ETA ≤ 閾值觸發通知、ETA > 閾值不觸發、同一站不重複觸發、權限被拒顯示提示、通知內容正確（路線名+站名+到站時間）
- [ ] 5.4 實作深色模式：跟隨系統 + 手動切換，選擇存 localStorage
