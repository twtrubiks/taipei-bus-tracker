...
### Requirement: 列出快捷
使用者執行 `bus-notify --list` 時，系統 SHALL 顯示所有已儲存的快捷。

#### Scenario: 有快捷
- **WHEN** 使用者執行 `bus-notify --list` 且有儲存的快捷
- **THEN** 系統列出每個快捷的名稱、路線、站名、方向、閾值

#### Scenario: 無快捷
- **WHEN** 使用者執行 `bus-notify --list` 且無快捷
- **THEN** 系統顯示「尚無快捷設定」
