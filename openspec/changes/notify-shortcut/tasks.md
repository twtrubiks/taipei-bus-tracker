...
- [x] 3.1 互動選站完成後，提示使用者輸入快捷名稱（Enter 跳過）
- [x] 3.2 同名快捷覆蓋確認：顯示現有設定，詢問 y/N

## 4. 快捷啟動

- [x] 4.1 實作快捷載入：讀取 notify.yaml 找到對應快捷，建構 Route/Stop 物件，直接呼叫 runMonitor
- [x] 4.2 處理錯誤情況：快捷不存在時列出可用快捷、notify.yaml 不存在或格式錯誤時顯示錯誤
